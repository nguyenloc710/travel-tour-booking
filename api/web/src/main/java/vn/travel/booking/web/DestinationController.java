...
import vn.travel.booking.application.destination.GetDestinationUseCase;
import vn.travel.booking.application.destination.ListDestinationsUseCase;
...
public class DestinationController implements DestinationsApi {
    ...
    private final ListDestinationsUseCase listDestinations;
    private final GetDestinationUseCase getDestination;

    public DestinationController(ListDestinationsUseCase listDestinations,
                                 GetDestinationUseCase getDestination) {
    ...
    public ResponseEntity<List<Destination>> listDestinations(
        ...
        List<Destination> than = listDestinations
                .execute(RequestScope.market(market), locale, region).stream()
    ...
    public ResponseEntity<Destination> getDestination(
        ...
        return phanHoi(locale).body(sang(
                getDestination.execute(RequestScope.market(market), locale, slug)));
    }
    ...
    private static Destination sang(DestinationSummary d) {
        return new Destination(
                d.slug(),
                d.name(),
                ProductMapper.sangRef(d.region()),
                d.productCount())
